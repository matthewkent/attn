...
S3Ajax.prototype = {
    ...
    putOLD: function (bucket, key, content/*, [params], cb, [err_cb]*/) {
        ...
        if (kwArgs.meta) {
            for (var k in kwArgs.meta) {
                if (kwArgs.meta.hasOwnProperty(k)) {
				 ...
				 if (kwArgs.meta && (kwArgs.content == null)) {
					hdrs['x-amz-copy-source'] = kwArgs.resource;
					special_meta += "\n";
					special_meta += "x-amz-copy-source:"+kwArgs.resource+"\n";
					hdrs['x-amz-metadata-directive'] = "REPLACE";
					meta_to_sign += "x-amz-metadata-directive:REPLACE\n";
					kwArgs.content_type = '';
				}
            ...
            if (req.readyState === 4) {
                ...
                if (req.responseXML && kwArgs.parseXML !== false) {
                    obj = _this.xmlToObj(req.responseXML, kwArgs.force_lists);
										obj["headers"] = this.getAllResponseHeaders();
                }

								if(kwArgs.method=="HEAD"){
									obj = {"Key" : kwArgs.resource};
									var lines = this.getAllResponseHeaders().split(/\r\n/);
									for(line in lines){
										var _header=lines[line];
										if((_header!=null) && (_header!="")){
											var matches = _header.match(/^(.+)\:\s+(.+)$/);
											if(matches){
    ...
    getXMLHttpRequest: function () {
		...
		linkable: function (bucket, key) {
				var kwArgs = { method : "GET", resource: '/' + bucket + '/' + key, content_type : "video/mp4"};
				
        var expires = Math.floor(((new Date()).getTime() / 1000)) + 86400;
        ...
        var signature  = this.hmacSHA1(s,this.secret_key);
				...
				return this.base_url + "/" + bucket + "/" + key + "?AWSAccessKeyId=" + this.key_id + "&Signature=" + encodeURIComponent(signature) + "&Expires=" + expires;
		},
		
		put: function(bucket, key, meta, content_type, body, cb){
			var _this = this;
      ...
      req.open("PUT", url, true);
			...
			s+= "x-amz-acl:public-read\n";
			var http_date = this.httpDate();
			req.setRequestHeader('x-amz-date', http_date);
			s+= "x-amz-date:"+http_date+"\n";
			for (var j in meta) { 
          if (meta.hasOwnProperty(j)) {
              req.setRequestHeader('x-amz-meta-'+j, meta[j]);
							s += 'x-amz-meta-'+j+':'+meta[j]+"\n";
          }
      }
			// req.setRequestHeader('x-amz-metadata-directive', "REPLACE");
			// s+= "x-amz-metadata-directive:REPLACE\n";
			s+=resource;
			var signature = this.hmacSHA1(s, this.secret_key);
			req.setRequestHeader('Authorization', "AWS "+this.key_id+":"+signature);
			req.onreadystatechange = function () {
				if (req.readyState === 4) {
					cb();
				}
